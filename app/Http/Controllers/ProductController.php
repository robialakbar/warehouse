...
class ProductController extends Controller
{
	...
	public function products_wip_history(Request $req){
		...
		$products = ProductWIP::with('getUserUpdate','getUserCreate')->leftJoin("products", "products_wip.product_id", "=", "products.product_id")
		->select("products_wip.*", "products.*");
	...
	public function product_check(Request $req){
        ...
        // $product = DB::table('products')->where([["product_code", $req->pcode], ["warehouse_id", $warehouse_id]])
		$product = Product::select("product_id", "product_code","product_name")->where('product_code', $req->pcode)->where('warehouse_id', $warehouse_id)->first();
		$endingTotalStockIn   = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $product->product_id], ["type", 1]])->sum("product_amount");
		$endingTotalStockOut  = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $product->product_id], ["type", 0]])->sum("product_amount");
		$endingTotalReturn  = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $product->product_id], ["type", 2]])->sum("product_amount");
		$endingTotalDefect  = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $product->product_id], ["type", 3]])->sum("product_amount");
		$result = ["status" => 0, "data" => null];

		if(!empty($product)){
			$product['availableStock'] = $endingTotalStockIn-$endingTotalStockOut-$endingTotalReturn-$endingTotalDefect;
	...
	public function product_wip_complete(Request $req){
		...
		$wip        = ProductWIP::select("*")->where([["product_wip_id", $wip_id], ["warehouse_id", $warehouse_id]])->first();
	...
	public function product_wip_import(Request $req){
		...
		$countImport = count($data);
		foreach($data as $d){
			$kode_product = $d["KODE PRODUK"];
			$product = Product::where('product_code', $kode_product)
			->first();
	...
	public function product_stock2(Request $req){
		if(!$req->has('item')){
			return back()->with('error', 'Silahkan Scan Product Terlebih Dahulu');
		}

		$data['stock_name']       = $req->name;
		$data['no_nota']    = $req->no_nota;
		$data['type']       = $req->type;
		...
		foreach ($req->item as $productId => $jumlahItem) {
			$product = Product::find($productId);
			$endingTotalStockIn   = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $productId], ["type", 1]])->sum("product_amount");
			$endingTotalStockOut  = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $productId], ["type", 0]])->sum("product_amount");
			$endingTotalDefect  = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $productId], ["type", 0]])->sum("product_amount");
	
			$endingAmount = $endingTotalStockIn-$endingTotalStockOut-$endingTotalDefect;
			$shelf = Stock::where('warehouse_id',$warehouse_id)->where('product_id', $productId)->orderBy('datetime', 'desc')->first();

			$data['product_id']     = $productId;
			$data['product_amount'] = $jumlahItem;
			$data['warehouse_id']   = $warehouse_id;
			$data['shelf_id']       = $shelf->shelf_id;
			$data["datetime"] = date("Y-m-d H:i:s");


			$data["ending_amount"] = $endingAmount - $jumlahItem;  
			$updateStock = DB::table('stock')->insertGetId($data);
			if($updateStock){
				$result = "Stok berhasil diupdate.";
			} else {
				$result =  "Stok gagal diupdate! Mohon coba kembali!";
			}
		}

		return redirect()->action('ProductController@product_stock_history')->with('success', $result);
	...
	public function deleteCheckbox(Request $request){
		...
		$del = Product::whereIn("product_id", [$request->id])->delete();

		if($del){
			$result = ["status" => 1, "message" => "Berhasil Menghapus!"];
		} else {
			$result = ["status" => 0, "message" => "Gagal Menghapus!"];
		}
	...
	public function defectStore(Request $req){
		if(!$req->has('item')){
			return back()->with('error', 'Silahkan Scan Product Terlebih Dahulu');
		}
		...
		foreach ($req->item as $productId => $jumlahItem) {
			...
			$endingTotalStockIn   = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $productId], ["type", 1]])->sum("product_amount");
			$endingTotalStockOut  = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $productId], ["type", 0]])->sum("product_amount");
			$endingTotalReturn  = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $productId], ["type", 2]])->sum("product_amount");
			$endingTotalDefect  = DB::table('stock')->where([["warehouse_id", $warehouse_id], ["product_id", $productId], ["type", 3]])->sum("product_amount");
			$endingAmount = $endingTotalStockIn-$endingTotalStockOut-$endingTotalReturn-$endingTotalDefect;
			$shelf = Stock::where('warehouse_id',$warehouse_id)->where('product_id', $productId)->orderBy('datetime', 'desc')->first();
