			...
			<div class="card-body">
				<table id="table" class="table table-sm table-bordered table-hover table-striped">
					...
					<tbody>
						...
						@php
						$data = ["user_id" => $d->id, "fullname" => $d->name, "username" => $d->username, "role" => $d->role, "is_show" => $d->is_show, "is_delete"=> $d->is_delete];
						if($d->role == 0){
							$role = "Admin";
						} else {
							$role = "User";
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form role="form" id="update" action="{{ route('users.save') }}" method="post">
						...
						<div class="form-group row">
							<label for="role" class="col-sm-4 col-form-label">Permision</label>
							<div class="col-sm-8">
								<div class="form-check form-check-inline">
									<input class="form-check-input" type="checkbox" name="is_show" id="is_show" value="Y">
									<label class="form-check-label" for="inlineCheckbox1">Lihat Harga Beli</label>
								</div>
								<div class="form-check form-check-inline">
									<input class="form-check-input" type="checkbox" name="is_delete" id="is_delete" value="Y">
									<label class="form-check-label" for="inlineCheckbox2">Hapus</label>
								</div>
							</div>
						</div>
					</form>
...
<script>
	function resetForm(){
		...
		$('#user_id').val('');
		$('#is_show').removeAttr('checked');
		$('#is_delete').removeAttr('checked');
	}
	...
	function editUser(data){
		...
		if(data.is_delete == 'Y'){
			$('#is_delete').attr('checked', 'checked');
		}
